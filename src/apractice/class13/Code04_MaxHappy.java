...
public class Code04_MaxHappy {
    ...
    public static int maxHappy2(Employee employee) {
        ...
        return Math.max(process(employee).yes, process(employee).no);
    }
    ...
    public static Info process(Employee e) {
        ...
        List<Employee> employeeList = e.nexts;
        if(employeeList != null && employeeList.size() > 0) {
            for(int i=0;i<employeeList.size();i++) {
                yes += process(employeeList.get(i)).no;
                no += Math.max(process(employeeList.get(i)).yes ,process(employeeList.get(i)).no);
            }
        }
